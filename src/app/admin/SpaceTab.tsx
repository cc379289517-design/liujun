...
export default function SpaceTab({ buildings, onRefresh }: SpaceTabProps) {
  ...
  async function deleteRoom(buildingId: number, roomId: number) {
    ...
    await fetch(`/api/buildings/${buildingId}/rooms?roomId=${roomId}`, { method: "DELETE" });
    onRefresh();
  }
  ...
  return (
    <div className="flex gap-4 h-[calc(100vh-140px)]">
      ...
      <div className="w-72 shrink-0 flex flex-col gap-3">
        ...
        <div className="flex-1 overflow-y-auto space-y-2 task-scroll">
          {buildings.map((b) => (
            <div
              key={b.id}
          ...
          <div className="space-y-4">
            ...
            <div className="card p-5">
              <div className="flex items-center justify-between mb-4">
                <h2 className="text-lg font-bold text-[--text-primary]">{selectedBuilding.name}</h2>
                <div className="flex items-center gap-2">
                  <span className="text-xs text-[--text-muted]">
                    {selectedBuilding.floorPlanUrl ? "已上传平面图" : "未上传平面图"}
                  </span>
                </div>
              </div>
                ...
                {uploading ? (
                  ...
                  <>
                    ...
                    <p className="text-xs text-[--text-secondary]">
                      {selectedBuilding.floorPlanUrl ? "拖拽或点击更换平面图" : "拖拽或点击上传平面图"}
                    </p>
                  </>
                )}
              </div>
            ...
            <div className="card p-5">
              ...
              <div className="space-y-1.5">
                ...
                {selectedBuilding.rooms.length === 0 ? (
                  <div className="text-center py-6 text-[--text-muted] text-sm">暂无房间</div>
                ) : (
                  selectedBuilding.rooms.map((room) => (
                    <div
                      key={room.id}
                      className="grid grid-cols-6 gap-3 px-3 py-2.5 rounded-xl bg-[--bg-base] hover:bg-gray-100 transition-colors items-center"
                    >
                      <span className="text-sm font-medium text-[--text-primary]">{room.roomNumber}</span>
  ...
  return (
    <div className="fixed inset-0 bg-black/30 backdrop-blur-sm flex items-center justify-center z-50" onClick={onClose}>
      <div className="card p-6 w-full max-w-md" onClick={(e) => e.stopPropagation()}>
        <h2 className="text-lg font-bold text-[--text-primary] mb-5">添加楼座</h2>
...
function RoomModal({
  ...
  return (
    <div className="fixed inset-0 bg-black/30 backdrop-blur-sm flex items-center justify-center z-50" onClick={onClose}>
      <div className="card p-6 w-full max-w-md" onClick={(e) => e.stopPropagation()}>
        <h2 className="text-lg font-bold text-[--text-primary] mb-5">添加房间</h2>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
